...
internal class Streamline_DeepDVC_Processor : DLLProcessor
{
    public override GameAssetType GameAssetType => GameAssetType.Streamline_DeepDVC;

    public override string NamePath => "sl_deepdvc";

    public override string ExpectedDLLName => "sl.deepdvc.dll";

    public override string[] ValidFileDescriptions => new string[]
    {
        "SL.DEEPDVC PLUGIN - PRODUCTION",
        "SL.DEEPDVC PLUGIN - NOT FOR PRODUCTION",
    };
    ...
    public Streamline_DeepDVC_Processor(List<DLLRecord> manifestDllRecords) : base(manifestDllRecords)
    {
    }
}